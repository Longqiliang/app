...
(function($){
	var wgtVer = null,
		checkUrl = "http://11wangjp.oss-cn-shanghai.aliyuncs.com/update.json",
		wgtCheckUrl = "",
		wgtUrl = "",
		wgtuUrl = '',
...
function checkUpdateData(j){
	...
	var curVer = plus.runtime.version,
	 ...
	 console.log(inf);
	if(inf){
		var srcVer = inf.version;
		//判断是否需要升级
		var vabort = plus.storage.getItem( keyAbort );
		if ( vabort && srcVer==vabort ) {
			// 忽略此版本
			return;
		}
//		if(true){
		if(compareVersion(curVer,srcVer)){
			...
			plus.nativeUI.confirm(inf.note,function(i){
				switch(i.index){
					...
					break;
					case 1:
					plus.storage.setItem( keyAbort, srcVer );
					plus.storage.setItem(keyUpdate, (new Date()).getTime().toString());
					break;
					case 2:
		...
		switch ( xhr.readyState ){			    
			case 4:
			    if( xhr.status == 200 ){
					...
					if(result.appid == plus.runtime.appid){
//	                if(true){
						//保存到本地文件中
